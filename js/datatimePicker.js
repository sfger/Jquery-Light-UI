(function($) {
    ...
    $.fn.slider = function(o){
        ...
        return this.each(function(){
            ...
            var that = this;
            var i = $this.index();
            var $galerys =$(op.galerys, this);
            var $switcher =$(op.switcher, this);
            var timer = function(){
                return setTimeout(function(){
                    if(that.hoverd){
                    }else{
                        var i = $switcher.filter('.cur').index();
                        $switcher.eq(i===$switcher.length-1 ? 0 : (i+1)).trigger('click');
                        that.timer = null;
                        that.timer = timer();
                    }
                }, op.timer);
            ...
            $(this).on({
                click: function(e){
                    var i = $(this).index();
                    $switcher.removeClass('cur').eq(i).addClass('cur');
                    $galerys.filter(':visible').fadeOut().end().eq(i).fadeIn();
                    return false;
                }
            }, op.switcher).on({
                mouseenter: function(){
    ...
    window.hotelIndex = window.hotelIndex || {
        init: function() {
            var $sdate = $('#sdate');
            var $edate = $('#edate');
            $sdate.datePicker({
                ...
                proxy:$("#page"),
                minDate: date("Y-m-d"),
                onComplete: function(pdate){
                    if( $edate.val()<=pdate ){
                        $edate.val(date("Y-m-d", strtotime(pdate)+86400));
                        $edate.trigger('focus');
                    }
                }
            });
            $edate.datePicker({
                monthNum:2,
                minDate: function(){
                    return date('Y-m-d', strtotime($('#sdate').val())+86400);
                }
            });
            $('.p-tabs').on({
                click: function(){
                    var $this = $(this);
                    if($this.find('.srd-icon').hasClass('disabled')) return false;
                    $this.closest('.p-tabs').find('li a').removeClass('cur').end().end().addClass('cur');
                    var i = $(this.parentNode).index();
                    $('.p-tab-ctn .p-panel').removeClass('selected').eq(i).addClass('selected');
                    return false;
                }
            }, 'li a');
            $('.p-choose').on({
                click: function(){
                    var $this = $(this);
                    var i = $this.index();
                    $this.closest('.p-choose').find('a').removeClass('open').eq(i).addClass('open');
                    $this.closest('.p-panel').find('.p-list').removeClass('selected').eq(i).addClass('selected');
                }
            }, 'a');
            $('.f-galery').slider({
                timer:3000,
                switcher: '.f-switcher a',
                galerys: '.galery-ctn img'
            });
            var $cc = $('#city-content');
            var $tcity = $('#tcity');
            $tcity.on({
                'click focus': function(){
                    ...
                    var $this = $(this);
                    this.hovered = true;
                    var ct = document.getElementById('cityType');
                    ct = ct==='abroad' ? 'abroad' : 'cn';
                    $cc.css({
                        top: $this.position().top + this.offsetHeight,
                        left: $this.position().left
                    });
                    $('#cityChooserType').val('firstView');
                    $cc.show().find('.layout-cityCountryPop').hide().filter('.' + ct).show().end().end().find('.mix-content').show();
                },
                'blur' : function(){
                    this.hovered = false;
                }
                ...
                'click': function(){
                    $tcity.get(0).hovered = true;
                    $tcity.trigger('focus');
                }
            });
            $('#page').on({
                click: function(e){
                    if(!$tcity.get(0).hovered){
                        $cc.hide();
                    }
                }
            });
            $('.layout-cityCountryPop').on({
                click: function(){
                    var that = this;
                    var i = $(this).index();
                    $(this).siblings().removeClass('tabCur').end().addClass('tabCur');
                    $(this).closest('.layout-cityCountryPop').find('.tabCont').hide().eq(i).show();
                }
            }, ' .nav-citypop li').on({
                click: function(){
                    $("#city-content").hide();
                    window.cityChooserTypeHandler(this);
                }
            }, 'a')
            $cc.on({
                click: function(){
            ...
            $('.more .expand-btn').on({
                click: function(){
                    ...
                    var $btn = $this.parent();
                    $cc.css({
                        top: $btn.position().top + $btn.get(0).offsetHeight,
                        left: $btn.position().left + 16
                    });
                    $cc.show().find('.layout-cityCountryPop').hide().filter('.' + ct).show().end().end().find('.mix-content').hide();
                    $('#cityChooserType').val('quickFind');
                    return false;
                }
            });
            $();
            $('.sec-city .more').click(function(){
                var that = this;
                var $this = $(this);
                var ct = document.getElementById('cityType');
                ct = ct==='abroad' ? 'abroad' : 'cn';
                $cc.show().find('.layout-cityCountryPop').hide().filter('.' + ct).show().end().end().find('.mix-content').hide();
                $cc.css({
                    top: $this.position().top + $this.get(0).offsetHeight,
                    left: $this.position().left + this.offsetWidth - $cc.get(0).offsetWidth
                });
                $('#cityChooserType').val('city');
                return false;
            });
            $('.sec-hotel .more').click(function(){
                var that = this;
                var $this = $(this);
                var ct = document.getElementById('cityType');
                ct = ct==='abroad' ? 'abroad' : 'cn';
